...
DWAPlanner::DWAPlanner() : Node("b_local_path_planner")
{
    ...
    this->declare_parameter("predict_time", 5.0); // 変更の余地あり
    this->declare_parameter("heading_cost_gain", 1.0);
    this->declare_parameter("velocity_cost_gain", 0.2);
    this->declare_parameter("distance_cost_gain", 2.0);
    this->declare_parameter("robot_radius", 0.25); // いらない
    this->declare_parameter("radius_margin", 0.1); // いらない
    this->declare_parameter("search_range", 3.0);  // 変更の余地あり
